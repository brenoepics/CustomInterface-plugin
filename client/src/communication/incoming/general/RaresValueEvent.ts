...
export default class RaresValueEvent implements IncomingMessage {
  parse(data: Data): void {
    App.interfaceManager.container.$store.commit(
      ...
      data.rares.map(
        (rare) =>
          new Rare(rare.id, rare.name, rare.image, rare.category, rare.cost),
      ),
    );
    App.interfaceManager.container.$store.commit(
      "rarevalues/setCategories",
      data.categories.map(
        (category) => new RareCategory(category.id, category.name),
      ),
    );
    App.interfaceManager.container.$store.commit(
      "rarevalues/setOwned",
